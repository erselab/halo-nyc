...
Run with the `analysis` conda env from the bayes_opt directory:
    /gpfs/fs1/home/scrowel3/miniforge3/envs/analysis/bin/python3 scripts/leg_offset_check.py
"""
...
def main():
    ...
    for fid in FLIGHTS:
        ...
        plt.savefig(f"figures/leg_offset_check_{fid}.png", bbox_inches="tight", dpi=110)
        plt.close(fig)
        print(f"plot -> figures/leg_offset_check_{fid}.png")

        fig, ax = plt.subplots(figsize=(7, 7))
        sc = ax.scatter(lon, lat, c=leg_id % 10, cmap="tab10", s=6)
        ax.set_title(f"{fid}: detected legs (n={n_legs})")
        plt.savefig(f"figures/leg_segmentation_{fid}.png", bbox_inches="tight", dpi=110)
        plt.close(fig)
        print(f"plot -> figures/leg_segmentation_{fid}.png")
